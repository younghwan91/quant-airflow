...
`docker-compose.timescale.yml`(리플리카 기동)과 `scripts/backup_to_gdrive.sh` 뿐이라
`git sparse-checkout set`으로 두 파일만 받는다. 전체 레포(DAG·수집기 등)는 항상 Airflow
호스트 쪽 clone 이 정본이다 — 같은 레포가 양쪽 호스트에 있다고 양쪽 다 "찐"인 게
아니라는 점에 주의(구성 파일 수정은 Airflow 호스트에서 하고 push, scalp-it 쪽은 pull 만).
