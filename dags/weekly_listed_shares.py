...
(**화요일**)만 수집한다. 컨테이너 기동(10:00) 10분 후, daily_collection_catchup
(10:05)과는 5분 겹치지 않게 스태거링. 월요일이 아니라 화요일인 이유는 아래
``schedule`` 옆 주석에 있다 — daily_short_credit 과 TR 버킷이 달라 겹쳐 돌리면
두 작업이 한 작업 시간에 끝난다.
