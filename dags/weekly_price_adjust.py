...
**평일 짝이 따로 있다.** 이 DAG 는 토요일에만 돌고, 그 사이 월~금에는 조정가
테이블에 그 주의 행이 없다(실측 2026-08-27: 4거래일 결측). 그래서
``daily_price_adjust``(평일 16:55)가 같은 재계산을 돌려 이번 주를 채운다.
이 토요일 런이 남아 있는 이유는 아래 ``ExternalTaskSensor`` 다 — 폐지 시세
백필이 끝난 뒤에 돌아야 새로 폐지된 종목의 이력이 조정가에 들어간다. 평일에는
기다릴 대상이 없다.
...
~44건) **조정계수**가 자주 바뀌지는 않는다 — 다만 그건 이번 주 행이 없어도
된다는 뜻이 아니다(위 참고). 실측 6분 23초 / 피크 5.19GB 라 하루 한 번은 저렴하다.
